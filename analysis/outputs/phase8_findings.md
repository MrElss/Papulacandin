...
ensembles. Phase 8 ran them (23 healthy ensembles, 132–597 conformers each;
PAPU-0078/6h gave a single conformer and is excluded from the primary set) and
...
| Descriptor vs log₁₀(serum MIC), n=23 | MMFF proxy (Ph7) | real CREST (Ph8) |
|---|---|---|
| hydrophobic_sasa_mean | −0.45 (p=0.029) | **−0.33 (p=0.123)** |
| rg_mean | −0.39 (p=0.062) | −0.37 (p=0.085) |
| hydrophobic_sasa_std (rigidity) | −0.16 (n.s.) | +0.11 (n.s.) |
| polar_sasa_mean | −0.16 (n.s.) | −0.09 (n.s.) |
...
0.78 → 100 µg/mL and by itself tracks serum MIC at **ρ = +0.77 (p < 0.001)**.
...
|---|---|---|
| hydrophobic_sasa_mean | **+0.03 (p=0.89)** | +0.27 (p=0.21) |
| hydrophobic_fraction_mean | +0.19 (p=0.39) | +0.28 (p=0.19) |
| hydrophobic_sasa_std | +0.00 (p=0.98) | −0.06 (p=0.78) |
| polar_sasa_mean | −0.31 (p=0.14) | −0.32 (p=0.13) |
| rg_mean | −0.23 (p=0.29) | −0.03 (p=0.88) |

Once serum-free potency is partialled out, **hydrophobic SASA's correlation with
serum MIC vanishes (ρ = 0.03)**. The apparent Phase-7 signal was compounds that
...
serum-tolerance endpoint — nothing is significant at n = 23, but the signs flip
to the **mechanistically expected** albumin direction:
- more exposed **hydrophobic** surface → **larger** shift (worse tolerance): ρ = +0.27
- more exposed **polar** surface → **smaller** shift (better tolerance): ρ = −0.32

**polar_sasa_mean** is the single most consistent and coherent lead (partial
−0.31, shift −0.32, both p ≈ 0.13): exposing polar rather than hydrophobic
...
- **Raw serum MIC is the wrong endpoint** — it is ~60% explained by intrinsic
  potency (ρ = 0.77). Future serum-tolerance modelling must use the serum SHIFT
  (or serum-free MIC as a covariate), as done here.
- **The honest state of the evidence:** the only coherent, non-confounded trend
  is "expose polar, not hydrophobic, surface" (polar SASA vs shift ρ = −0.32),
...
- n = 23 (6h excluded: its `--noreftopo` retry produced a single conformer;
  structure verified intact but no ensemble — see phase8 logs). Including 6h as a
  single-point (n = 24) does not change any conclusion (hydrophobic SASA ρ = −0.36,
  still n.s.; partial correlations unchanged in sign).
